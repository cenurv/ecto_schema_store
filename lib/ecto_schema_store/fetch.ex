defmodule EctoSchemaStore.Fetch do
  ...
  defp __preload__(query, []), do: query
  defp __preload__(query, preload) do
    from m in query,
    preload: ^preload
  end
  ...
  def all(filters, opts) do
    ...
    order_list = Keyword.get opts, :order_by, nil
    build_query = Keyword.get opts, :build_query
    repo = Keyword.get opts, :repo
    ...
    quote do
      ...
      * `order_by`             - Order the results by a the provided keyword list.
      ...
      def all(filters, opts \\ []) do
        opts =
          opts
          |> Keyword.put(:build_query, &build_query/1)
          |> Keyword.put(:repo, unquote(repo))

        EctoSchemaStore.Fetch.all(filters, opts)
      end
      ...
      * `order_by`             - Order the results by a the provided keyword list.
      ...
      def one(filters, opts \\ []) do
        ...
        EctoSchemaStore.Fetch.one(filters, opts)
      end
      ...
      """
      defdelegate to_map(record), to: EctoSchemaStore.Fetch
    end
  end
end
